...
def plot_single_scenario(scenario: dict):
    ...
    if plot_loss:
        ...
        if args.latex:
            print("\nLaTeX Loss:")
            for app in apps:
                latex_plot(app['time'], app['loss'], label=app['name'])
    ...
    if plot_latency:
        ...
        if args.latex:
            print("\nLaTeX Latency:")
            for app in apps:
                latex_plot(app['time'], app['latency'], label=app['name'])
    ...
    if len(apps) == 1:  # If there is only one application, plot everything for it
        ...
        if args.latex:
            print("\nLaTeX Sendrate:")
            for app in apps:
                latex_plot(app['time'], app['sendrate'], label=app['name'])
    ...
    if plot_active_paths:
        ...
        if args.latex:
            print("\nLaTeX Active Paths:")
            for app in apps:
                latex_plot(app['time'], app['active_path'], label=app['name'], sharp=True)
