...
VideoConferenceApp::rescore (double active_loss)
{
  return active_loss > acceptable_loss ||
         (active_loss == 0 &&
          best_path_id / num_paths <
              2); // also rescore when loss is zero and not highest quality to select an increased quality.
}
...
double
VideoConferenceApp::ComputeScore (double latency, double loss, double additional_scoring,
  ...
  if ((loss > acceptable_loss && quality > 0) ||
      (best_path_id != -1 &&
       ((quality > 1 + best_path_id / num_paths) ||
        ...
        (quality > 0 && quality >= best_path_id / num_paths &&
         path_infos->at (best_path_id).activeLoss > acceptable_loss))))
    {
...
VideoConferenceApp::ComputeAllScores (bool triggered_by_probes)
{
  ...
  selected_qualities.push_back (
      std::make_tuple (host->GetLocalTime ().ToInteger (Time::Unit::MS), quality, best_path_id));
}
  ...
  for (auto entry : app_responses)
    {
      ...
      if (i + 1 < selected_qualities.size () &&
          timestamp > std::get<0> (selected_qualities.at (i + 1)))
        {
          ++i;
        }
      uint32_t quality = std::get<1> (selected_qualities.at (i));
      int32_t selected_path = std::get<2> (selected_qualities.at (i));
      double score = VideoConferenceApp::MetricScore (entry.second.avg_latency / 1000.,
                                                      entry.second.loss, quality);
      std::cout << timestamp << "(" << entry.first.ToDouble (Time::Unit::MIN) << " min), "
                << entry.second.avg_latency / 1000. << ", " << entry.second.loss << ", "
                << entry.second.bytes_received << ", " << selected_path << ", " << quality << ", "
                << score << std::endl;
    }
                          ...
                          bool was_active)
{
  ...
  if ((quality > 0 && !isActivePath (path_id)) || (quality > 1 + best_path_id / num_paths) ||
      (quality == 1 + best_path_id / num_paths && path_infos->at (best_path_id).activeLoss > 0))
    {
                        ...
                        bool was_active)
{
  ...
  if ((quality > 0 && !current_path) || (quality > 1 + best_path_id / num_paths) ||
      (quality == 1 + best_path_id / num_paths && path_infos->at (best_path_id).activeLoss > 0) ||
      ...
      for (uint16_t j = 0; j < hops.size (); j++)
        {
          ...
          auto hop = hops.at (j);
          if (GET_HOP_ISD (hop) != given_hop.at (0) || GET_HOP_AS (hop) != given_hop.at (1) ||
              GET_HOP_ING_IF (hop) != given_hop.at (2) || GET_HOP_EG_IF (hop) != given_hop.at (3))
            {
              found = false;
              break;
            }
