      ...
      if (packet->payload_type == PayloadType::QOS_PROBE_REQ)
        {
          ...
          ProcessQosProbeReq (local_if_to_send, packet, true, 0, packet->cur_hopf,
                              packet->curr_inf);
        }
                                  ...
                                  uint16_t as_if_to_send, uint16_t old_hopf, uint16_t old_inf)
{
  ...
  if (predicted_new_throughput.at (local_if).size () > 0 &&
      estimated_packetloss.at (local_if).size () > 0)
    {
      double estimated_loss =
          estimated_packetloss.at (local_if).at (estimated_packetloss.at (local_if).size () - 1);
      uint64_t predicted_new_throughput_val = predicted_new_throughput.at (local_if).at (
          predicted_new_throughput.at (local_if).size () - 1);
      int64_t new_bwd = predicted_new_throughput_val + request.expected_bandwidth;
      new_loss =
          new_bwd < avail_bwd_bytes && estimated_packetloss.at (local_if).at (
                                           estimated_packetloss.at (local_if).size () - 1) == 0
              ? 0
              : estimated_loss + ((double) request.expected_bandwidth) / avail_bwd_bytes;

      predicted_new_throughput_val +=
          request.expected_bandwidth /
          5; // assume conservatively that 20% will send traffic through here
      predicted_new_throughput.at (local_if).at (predicted_new_throughput.at (local_if).size () -
                                                 1) = predicted_new_throughput_val;
