                             ...
                             uint32_t runtime_config)
{
  ...
  if (!all_paths.empty ())
    {
      std::cout << "Found paths from AS " << ia_addr << " to " << dst_ia << std::endl;
  ...
  // TODO: pass this via the runtime config
  bool use_bgp_lb_path = true;

  if (use_bgp_lb_path)
    {
      nlohmann::json events_json;
      std::ifstream events_file ("configs/traffic-engineering/toy.json");
      events_file >> events_json;
      events_file.close ();

      // Look up the path using the src as, dest as, src host, dest host and app_id
      std::string key = std::to_string (ia_addr) + "-" + std::to_string (dst_ia) + "-" +
                        std::to_string (local_address) + "-" + std::to_string (dst_host) + "-" +
      ...
      // Print all paths
      for (auto const &path : events_json["paths"])
        {
          std::cout << "Path: " << path << std::endl;
        }

      // If this key cannot be found (e.g. as TCP receiver), try to find the
      // reverse path instead
      if (events_json["paths"].find (key) == events_json["paths"].end ())
        {
          key = std::to_string (dst_ia) + "-" + std::to_string (ia_addr) + "-" +
                std::to_string (dst_host) + "-" + std::to_string (local_address) + "-" +
                std::to_string (app_id);
          if (events_json["paths"].find (key) == events_json["paths"].end ())
            {
              NS_FATAL_ERROR ("Path not found in events file for app "
                              << app_id << " (AS " << ia_addr << " to " << dst_ia << ")");
            }
          else
            {
              seg->reverse = true;
      ...
      for (auto const &hop : events_json["paths"][key]["hops"])
        {
      ...
      if (seg->reverse)
        {
          // If the path is reversed, reverse the hops,
          // but egress and ingress stay the same
          std::reverse (seg->hops.begin (), seg->hops.end ());
        }

      bgp_path.push_back (seg);

      all_paths.clear ();
      all_paths.push_back (bgp_path);

      std::cout << "Found path override for app " << app_id << " (AS " << ia_addr << " to "
                << dst_ia << "): ";
      PrintPath (bgp_path);
    }
