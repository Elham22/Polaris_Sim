...
* Declaring parameters and parsing the command line using ``ns3::CommandLine``.
  ...
  ::
    ...
    string beacon_server ("wifi-default");
    string runID;
    ...
    cmd.AddValue("beacon_server",   "Identifier for beacon_server.",                   beacon_server);
...
* Configuring the data and statistics to be collected.  The basic paradigm is that an ``ns3::DataCollector`` object is created to hold information about this particular run, to which observers and calculators are attached to actually generate data.  Importantly, run information includes labels for the ''experiment'', ''beacon_server'', ''input'', and ''run''.  These are used to later identify and easily group data from multiple trials.

  * The experiment is the study of which this trial is a member.  Here it is on WiFi performance and distance.
  * The beacon_server is the code or parameters being examined in this trial.  In this example it is fixed, but an obvious extension would be to investigate different WiFi bit rates, each of which would be a different beacon_server.
  ...
  * The runID is a unique identifier for this trial with which it's information is tagged for identification in later analysis.  If no run ID is given the example program makes a (weak) run ID using the current time.
  ...
  ::
    ...
    DataCollector data;
    data.DescribeRun(experiment, beacon_server, input, runID);
...
.. sourcecode:: text
  ...
  attr beacon_server "wifi-default"
  attr input "50"
  attr description ""
